...
**Version:** 1.9  
**Last Updated:** November 26, 2025 (Number Input Component pattern, custom +/- buttons)  
...
### Token Pages with Logo

Token pages (DGKO, BABYDGKO) include the token logo and follow this specific pattern:

```tsx
<div className="mb-12">
  <div className="flex items-center gap-4 mb-3">
    <TokenImage assetId={TOKEN_IDS.TOKEN_NAME} size="xl" />
    <h1 className="text-5xl font-medium text-white">Token Name</h1>
  </div>
  <p className="text-xl text-gray-400">Token description</p>
</div>
```

**Specifications:**
- **Container:** Same `mb-12` as standard pages
- **Logo + Title Row:** `flex items-center gap-4 mb-3`
- **Token Logo:** `size="xl"` (64px) using `TokenImage` component with correct `assetId` prop
- **Title:** Same `text-5xl font-medium text-white` as standard pages
- **Description:** Same `text-xl text-gray-400` as standard pages

**CRITICAL: All token pages MUST use identical sizing and spacing:**
- ✅ Title: `text-5xl` (not `text-4xl`, not `text-6xl`)
- ✅ Description: `text-xl` (not `text-base`, not `text-lg`)
- ✅ Container margin: `mb-12` (not `mb-8`, not `mb-16`)
- ✅ Title-description gap: `mb-3` (not `mb-2`, not `mb-4`)
- ✅ Logo-title gap: `gap-4` (not `gap-6`, not `gap-3`)
...
**Design Principle:**
Token pages can have personality (like BABYDGKO's sparkles icon), but their fundamental structure (sizing, spacing, layout) must remain consistent with other token pages. Personality comes through content and small decorative elements, not through different sizes or spacing.

**Anti-patterns (DO NOT USE):**
- ❌ Different title sizes between token pages (`text-5xl` on DGKO, `text-4xl` on BABYDGKO)
- ❌ Different description sizes between token pages
- ❌ Different spacing values between token pages
- ❌ Using `tokenId` prop instead of `assetId` prop on TokenImage
- ❌ Using invalid size values like `size="large"` (use `"xl"` instead)

**Exception:**
Small decorative elements (like sparkles icon) can be added for personality, but must not affect the sizing or spacing consistency.
...
### Problem: Default Browser Spinners
Browser default number input spinners (+/-) are:
- ❌ Inconsistent across browsers
- ❌ Don't match our design system
- ❌ Break glass morphism aesthetic
- ❌ Poor UX on touch devices

### Solution: Custom Number Input Component

We created a `NumberInput` component with beautifully designed increment/decrement buttons that follow our design guide.

**Full NumberInput Component (with +/- buttons):**
```tsx
...
- ✅ Smooth hover states
- ✅ Keyboard accessible
- ✅ Optional MAX button integration
- ✅ Min/max constraints
- ✅ Disabled state support

**When to use:**
- Staking amount inputs (where users adjust frequently)
- Any input where +/- buttons provide value
- Forms with constrained numeric ranges

**Simple Input (spinner hidden only):**

For streamlined interfaces like swap or send forms, hide spinners with CSS:
...
<input
  ...
  placeholder="0.00"
  className="bg-transparent text-2xl font-mono text-white outline-none
...
**Design Specifications for Custom Buttons:**

Increment/Decrement Buttons:
- Size: `w-7 h-6` (28×24px)
- Border radius: `rounded-lg` (8px)
- Background: `bg-white/5` → `hover:bg-digiko-primary/20`
- Border: `border-white/10` → `hover:border-digiko-primary/40`
- Icon color: `text-gray-400` → `hover:text-digiko-primary`
- Stack vertically with `gap-1` (4px)
- Transition: `duration-300`
- Disabled: `opacity-30` + no hover effects

Container:
- Background: `bg-klever-dark`
- Border: `border-gray-700`
- Focus ring: `focus-within:ring-2 focus-within:ring-digiko-primary`
- Padding: `px-4 py-3`
- Border radius: `rounded-xl`

MAX Button (when used):
- Background: `bg-digiko-primary/20` → `hover:bg-digiko-primary/30`
- Border: `border-digiko-primary/30` → `hover:border-digiko-primary/50`
- Color: `text-digiko-primary`
- Hover: `scale-105`

**Usage Across App:**
- **Staking:** Full NumberInput with +/- buttons
- **Swap:** Simple input (spinners hidden)
- **Send:** Simple input (spinners hidden)
